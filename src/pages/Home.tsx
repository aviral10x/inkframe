      ...
      <section className="hero-grid ink-hero">
        ...
        <AnimatedSection delay={0.25}>
          <div className="hero-proof-panel">
            <DriveEmbed title={proofOfWork[0].title} driveId={proofOfWork[0].driveId} />
            <div className="hero-proof-meta">
              <span className="metadata">PROOF OF WORK</span>
              <p>{proofOfWork[0].description}</p>
            </div>
          </div>
        </AnimatedSection>
      </section>
      ...
      <section className="section-band">
        ...
        <div className="work-grid">
          {proofOfWork.map((project, index) => (
            <AnimatedSection key={project.id} delay={index * 0.05} className={`work-card work-card-${project.size}`}>
              <div className="work-media-shell" style={{ borderColor: `${project.accent}55` }}>
                <DriveEmbed title={project.title} driveId={project.driveId} />
              </div>
              <div className="work-card-copy">
                <span className="metadata" style={{ color: project.accent }}>{project.category} · {project.format}</span>
                <h3>{project.title}</h3>
